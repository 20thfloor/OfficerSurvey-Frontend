...
import { Questionlooks, Choices } from './UpdatePoleStyle'
...
import { updateEmployeeSurvey, getSurvey } from '../../../utils/apiUrls'
...
const UpdatePole = props => {
  ...
  const updatePole = async () => {
    ...
    const res = await putAPIWrapper(`${updateEmployeeSurvey} ${poleId}`, survey)
    if (!res.isError) {
  ...
  const fetchSurvey = async () => {
    ...
    const response = await getAPI(`${getSurvey} ${poleId}`, query)
    if (!response.isError) {
  ...
  return (
    <>
      <OfficerLoader isFetching={surveyFetchingSpinner}>
        ...
        <Questions item={survey.questions[0]} />
