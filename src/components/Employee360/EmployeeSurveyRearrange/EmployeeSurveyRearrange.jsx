...
import { getSurvey, reArrangeEmployeeFeedback } from '../../../utils/apiUrls'
import CardDrag from '../DragDropEmployeeSurvey/Card'
import update from 'immutability-helper'
import { DragnDrop } from './RearrangeStyle'

const EmployeeSurveyRearrange = () => {
  ...
  const fetchSurvey = async () => {
    ...
    const response = await getAPI(`${getSurvey} ${surveyId}`)
    if (!response.isError) {
  ...
  const rearrange = async () => {
    ...
    const response = await postAPIWrapper(`${reArrangeEmployeeFeedback}${surveyId}/`, {
      questions: questionsArray
    })

    if (!response.isError) {
      setshowSpinner(false)
      history.push(`/edit-employee-survey/${surveyId}`)
      return
    } else {
      setshowSpinner(false)
