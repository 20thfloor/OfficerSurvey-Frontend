...
import { getCommunitySurvey, updateCommunitySurvey } from '../../../utils/apiUrls'
...
import { Questionlooks, Required, Choices } from './UpdateCommunitySurveyStyle'
...
const UpdateCommunitySurvey = props => {
  UpdateCommunitySurvey.propTypes = {
  ...
  const submitSurvey = async () => {
    setShowSpinner(true)
    const res = await putAPIWrapper(`${updateCommunitySurvey} ${surveyId}`, survey)
    if (!res.isError) {
      ...
      <OfficerLoader isFetching={surveyFetchingSpinner}>
        ...
        <div className="d-flex justify-content-end pt-3">
          <OfficerButton
            ...
            color="officer"
            variant="large"
            click={() => history.push(`/community-survey-rearrange/${surveyId}`)}
          />
        ...
        {survey.questions.map((item, count) => {
          return (
            <div key={`data-comunity-update${count + 2}`}>
          ...
          <>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <OfficerAutocomplete
                  id="combo-box-community-update-type"
                  options={typeList}
            ...
            <Questions count={1} item={newQuestionData} />
          </>
        }
      />
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <OfficerAutocomplete
                  ...
                  disabled
                  defaultValue={updateQuestion.type}
                  label="Select Type"
                  idTextField="Select Type"
                  name="type"
                  onChange={(event, newValue) => {
