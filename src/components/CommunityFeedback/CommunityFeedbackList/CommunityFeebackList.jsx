...
import { getCommunitySurvey, deleteCommunitySurveyApi } from '../../../utils/apiUrls'
import { TableBody, DeleteHeading, CommunityFeedbackHeading } from './CommunityFeedbackStyle'
...
const CommunityFeedbackList = props => {
  const location = useLocation()

  CommunityFeedbackList.propTypes = {
  ...
  const openSurvey = () => {
    history.push(`create-community-survey/${category}`)
  }
  const fetchSurveys = async () => {
    setTableLoadSpinner(true)
    const response = await getAPI(getCommunitySurvey, query)
    if (!response.isError) {
    ...
    surveys.data.map(item => {
        ...
        <div className="d-flex justify-content-center ">
          <div className="pr-2">
            <OfficerButton
              ...
              variant="small"
              click={() => {
                history.push(`community-survey-report/${item.id}`)
              }}
            />
          </div>
      ...
      render.push(
        <div className="d-flex justify-content-center">
          <div className="pr-2">
            <OfficerCopyButton
              text={`${process.env.REACT_APP_DEPLOYED_LINK}/community-survey-by-link/${item.link}`}
              notify={props.notify}
            />
          </div>
          <div className="pr-2">
            <OfficerButton
              ...
              click={() => {
                item.type === 'Poll'
                  ? history.push(`update-community-pole/${item.id}`)
                  : history.push(`update-community-survey/${item.id}`)
              }}
            />
          </div>
          <OfficerButton
  ...
  const onClickDelete = async () => {
    ...
    const response = await deleteAPIWrapper(`${deleteCommunitySurveyApi} ${deleteItem.id}`)
    if (!response.isError) {
      fetchSurveys()
  ...
  const openPole = () => {
    history.push(`create-pole-community/${category}`)
  }

  return (
    <div>
      <CommunityFeedbackHeading>{location.state.detail}</CommunityFeedbackHeading>
      ...
      <OfficerTable
        ...
        headers={['Title', 'Response', 'Expiry Date', 'Responses Action', 'Survey Actions']}
        data={surveyListData}
        pagination
...
export default CommunityFeedbackList
