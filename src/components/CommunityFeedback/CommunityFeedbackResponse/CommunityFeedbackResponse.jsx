...
import { addCommunitySurveyResponse } from '../../../utils/apiUrls'
import { useParams } from 'react-router-dom'
import { OfficerCard } from '../../Common'
import { Question, QuestionNumber, Res, Answer, File } from './CommunityFeedbackResponseStyle'
...
const CommunityFeedbackResponse = () => {
  ...
  const fetchResponseDetail = async () => {
    ...
    const res = await getAPI(`${addCommunitySurveyResponse}${responseId}`, query)
  ...
  const showReviews = () => {
    ...
    if (info.data.length > 0) {
      for (var i = 0; i < info.data[0].questions_response.length; i = i + 2) {
        List.push(
          <Grid key={`data-response-community${i}`} container direction="row" justify="space-around" spacing={5}>
            ...
            {info.data[0].questions_response[i + 1] ? (
              <Grid key={`data-item-community${i + 1}`} item xs={12} sm={12} md={6}>
                <OfficerCard mt="0px" mb="0px" shouldFullHeight>
                  <div>
                    <Question>
